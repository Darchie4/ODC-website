    ...
    <div class="container">
        <div class="row g-3">
            <div class="col-3">
                ...
                @endif
                <h3>Rediger Bestyrelses medlem</h3>
                <div class="row g3">
                    <div class="col-4">
            ...
            <div class="col-8">
                <h1>Rediger bestyrelses medlem</h1>
                <form class="row g-3" action="{{route("admin.board.doUpdate", ['boardMember' => $boardMember])}}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="col">
                        <label for="name"><b>Navn</b> <b class="text-danger">*</b></label><br>
                        <input value="{{old('name', $boardMember->name)}}" class="form-control" id="name"
                               name="name" type="text" required>
                        <br>
                        ...
                        <select class="form-control" name="boardTitle">
                            @foreach($boardTitles as $boardTitle)
                                <option value="{{$boardTitle->id}}" {{$boardTitle->id == $boardMember->boardTitle->id ? 'selected' : ''}}>{{$boardTitle->name}}</option>
                            @endforeach
                        </select>
                        <br>
                    ...
                    <div class="col">
                        ...
                        <input type="number" min="{{now()->year-4}}" max="{{now()->year}}" step="1" value="{{old('elected_year', $boardMember->elected_year)}}" class="form-control" id="name"
                               name="elected_year" required>
                        <br>

                        <label for="description"><b>Kort beskrivelse</b> <b class="text-danger">*</b></label><br>
                        <textarea class="form-control" id="name" name="description" required>{{old('description', $boardMember->description)}}</textarea>

                    </div>
                    <button class="btn btn-success " type="submit"
                            value="Submit">Rediger</button>
                </form>
            </div>
        </div>
    </div>
